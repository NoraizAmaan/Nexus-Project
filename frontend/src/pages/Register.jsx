...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    setMsg(`✅ ${t('auth.registerSuccess') || 'User registered successfully. Redirecting to login...'}`);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-100 dark:from-slate-900 dark:to-slate-950 transition-colors duration-200">
      <form
        onSubmit={submit}
      ...
      >
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">{t('auth.createAccount')}</h2>
          <p className="text-gray-500 text-sm mt-1 dark:text-gray-400">{t('auth.joinUs')}</p>
        </div>

        {/* Name */}
        <div>
          <label className="text-sm font-medium text-gray-600 dark:text-gray-300">{t('auth.fullName')}</label>
          <input
            value={name}
          ...
          {!isNameValid && name && (
            <p className="text-xs text-red-500 mt-1">
              {t('auth.nameMinLength') || 'Name must be at least 3 characters'}
            </p>
          )}
        </div>
